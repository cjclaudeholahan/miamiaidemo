                      ...
                      <div>
                        ...
                        {openSec[`${co.name}_dcf`]&&(
                          <div className="mt-2 bg-white border border-blue-100 rounded-lg p-3 overflow-x-auto">
                            ...
                            <table className="w-full text-xs border-collapse min-w-max border-2 border-black">
                              ...
                              <tbody>
                                {[["Revenue",r=>fmtM(r.rev),{bold:true,topBorder:true}],["Rev Growth",r=>r.yr===1?"—":`${r.revGrowth}%`,{italic:true}],["EBITDA",r=>fmtM(r.ebitda),{topBorder:true,bg:true}],["EBITDA Margin",r=>`${r.ebitdaMargin}%`,{italic:true,bg:true}],["(-) SBC",r=>fmtM(r.sbc),{}],["SBC % Rev",r=>`${r.sbcPctRev}%`,{italic:true}],["EBITDA Post-SBC",r=>fmtM(r.ebitdaPostSBC),{bold:true,topBorder:true,bg:true}],["Post-SBC Margin",r=>`${r.ebitdaPostSBCMargin}%`,{italic:true,bg:true}],["(-) D&A",r=>fmtM(r.da),{}],["D&A % Rev",r=>`${r.daPctRev}%`,{italic:true}],["EBIT",r=>fmtM(r.ebit),{topBorder:true,bg:true}],["EBIT Margin",r=>`${r.ebitMargin}%`,{italic:true,bg:true}],["(-) Taxes",r=>fmtM(r.taxes),{}],["Tax Rate",r=>`${r.taxRate}%`,{italic:true}],["NOPAT",r=>fmtM(r.nopat),{topBorder:true,bg:true}],["NOPAT Margin",r=>`${r.nopatMargin}%`,{italic:true,bg:true}],["(+) D&A",r=>fmtM(r.da),{}],["(+/-) Other Cash Flow",r=>fmtM(r.otherCF),{}],["Other % Rev",r=>`${r.otherCFPctRev}%`,{italic:true}],["UFCF",r=>fmtM(r.ufcf),{bold:true,topBorder:true,bg:true}],["UFCF Margin",r=>`${r.ufcfMargin}%`,{italic:true,bg:true}],["UFCF % Conversion",r=>`${r.ufcfConv}%`,{italic:true,bg:true}],["PV of UFCF",r=>fmtM(r.pv),{blue:true,spacer:true}]].map(([lbl,fn,opts])=>(
                                  <tr key={lbl} className={`${opts.topBorder?"border-t-2 border-black":""} ${opts.bg?"bg-blue-50":""} ${opts.spacer?"border-t-8 border-transparent":""}`}>
                                    <td className={`px-2 py-1 text-gray-700 sticky left-0 whitespace-nowrap ${opts.bold?"font-bold":""} ${opts.italic?"italic text-gray-400":"font-medium"} ${opts.blue?"text-blue-700":""} ${opts.bg?"bg-blue-50":"bg-white"}`}>{lbl}</td>

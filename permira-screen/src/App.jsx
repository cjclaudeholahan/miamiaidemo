...
function runDCF(companyName,growthPct,ebitdaPct,endMarginPct,wacc,pgr,ntmRev){
  ...
  for(let yr=1;yr<=DCF_YEARS;yr++){
    ...
    rows.push({yr,label,rev:Math.round(rev),revGrowth:Math.round(revGrowth*10)/10,
      ...
      ufcf:Math.round(ufcf),pv:Math.round(pv)});
  }
...
const RAW=[
  // ── PURE-PLAY VSaaS ──
  {name:"Veeva Systems",vertical:"Healthcare",bucket:"Pure-Play VSaaS",hq:"US",sor:true,seat:true,pricing:"Seat-Based",peOwned:false,tev:22286,ntmRev:3668,growth:13,gm:77,ebitda:45,cagr:13,ntmRevX:6.1,peFit:"Medium",aiRisk:"Low",avoid:false,ltmEbitda:1484,pct52w:0.57,
  ...
  {name:"Bentley Systems",vertical:"Construction & Design SW",bucket:"Pure-Play VSaaS",hq:"US",sor:true,seat:true,pricing:"Seat-Based",peOwned:false,tev:11855,ntmRev:1744,growth:13,gm:82,ebitda:36,cagr:12,ntmRevX:6.8,peFit:"Medium",aiRisk:"Low",avoid:false,ltmEbitda:547,pct52w:0.60,
   ...
   sd:{sharePrice:35.12,sharesOut:306.3,marketCap:10757,netDebt:1098},
  ...
  {name:"Nemetschek",vertical:"Construction & Design SW",bucket:"Pure-Play VSaaS",hq:"DE",sor:true,seat:false,pricing:"Usage-Based",peOwned:false,tev:8677,ntmRev:1604,growth:13,gm:97,ebitda:33,cagr:14,ntmRevX:5.4,peFit:"Medium-High",aiRisk:"Low",avoid:false,ltmEbitda:447,pct52w:0.46,
   ...
   sd:{sharePrice:73.40,sharesOut:116.3,marketCap:8534,netDebt:143},
  ...
  {name:"CCC Intelligent Solutions",vertical:"Financial Services",bucket:"Pure-Play VSaaS",hq:"US",sor:true,seat:false,pricing:"Usage-Based",peOwned:false,tev:5103,ntmRev:1179,growth:9,gm:78,ebitda:42,cagr:9,ntmRevX:4.3,peFit:"High",aiRisk:"Low",avoid:false,ltmEbitda:447,pct52w:0.60,
   ...
   sd:{sharePrice:6.00,sharesOut:657.7,marketCap:3946,netDebt:1157},
  ...
  {name:"nCino",vertical:"Financial Services",bucket:"Pure-Play VSaaS",hq:"US",sor:true,seat:false,pricing:"Usage-Based",peOwned:false,tev:1949,ntmRev:650,growth:8,gm:68,ebitda:25,cagr:9,ntmRevX:3.0,peFit:"Medium",aiRisk:"Medium",avoid:false,ltmEbitda:138,pct52w:0.46,
   ...
   sd:{sharePrice:14.98,sharesOut:121.6,marketCap:1821,netDebt:128},
                        ...
                        {openSec[`${co.name}_dcf`]&&(
                          <div className="mt-2 bg-white border border-blue-100 rounded-lg p-3 overflow-x-auto">
                            ...
                            <table className="w-full text-xs border-collapse min-w-max">
                              ...
                              <tbody>
                                {[["Revenue",r=>fmtM(r.rev),{bold:true,topBorder:true}],["Rev Growth",r=>r.yr===1?"—":`${r.revGrowth}%`,{italic:true}],["EBITDA",r=>fmtM(r.ebitda),{}],["EBITDA Margin",r=>`${r.ebitdaMargin}%`,{italic:true}],["(-) SBC",r=>fmtM(r.sbc),{}],["SBC % Rev",r=>`${r.sbcPctRev}%`,{italic:true}],["EBITDA Post-SBC",r=>fmtM(r.ebitdaPostSBC),{bold:true,topBorder:true}],["Post-SBC Margin",r=>`${r.ebitdaPostSBCMargin}%`,{italic:true}],["(-) D&A",r=>fmtM(r.da),{}],["D&A % Rev",r=>`${r.daPctRev}%`,{italic:true}],["EBIT",r=>fmtM(r.ebit),{}],["EBIT Margin",r=>`${r.ebitMargin}%`,{italic:true}],["(-) Taxes",r=>fmtM(r.taxes),{}],["Tax Rate",r=>`${r.taxRate}%`,{italic:true}],["NOPAT",r=>fmtM(r.nopat),{}],["NOPAT Margin",r=>`${r.nopatMargin}%`,{italic:true}],["(+) D&A",r=>fmtM(r.da),{}],["(+/-) Other Cash Flow",r=>fmtM(r.otherCF),{}],["Other % Rev",r=>`${r.otherCFPctRev}%`,{italic:true}],["UFCF",r=>fmtM(r.ufcf),{bold:true,topBorder:true,blue:true}],["PV of UFCF",r=>fmtM(r.pv),{blue:true}]].map(([lbl,fn,opts])=>(
                                  <tr key={lbl} className={opts.topBorder?"border-t-2 border-black":""}>
                                    <td className={`px-2 py-1 text-gray-700 sticky left-0 bg-white whitespace-nowrap ${opts.bold?"font-bold":""} ${opts.italic?"italic text-gray-400":"font-medium"} ${opts.blue?"text-blue-700":""}`}>{lbl}</td>
                                    {co.dcf.rows.map(r=><td key={r.yr} className={`px-2 py-1 text-center ${opts.bold?"font-bold":""} ${opts.italic?"italic text-gray-400":""} ${opts.blue?"text-blue-700 font-medium":""}`}>{fn(r)}</td>)}
                                  </tr>
                                ))}
                              </tbody>
                            </table>

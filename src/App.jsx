                        ...
                        {openSec[`${co.name}_dcf`]&&(
                          <div className="mt-2 bg-white border border-blue-100 rounded-lg p-3 overflow-x-auto">
                            ...
                            <table className="w-full text-xs border-collapse min-w-max">
                              <thead><tr className="bg-gray-50"><th className="border border-gray-200 px-2 py-1.5 text-left font-semibold text-gray-600 sticky left-0 bg-gray-50 w-36">Metric</th>{co.dcf.rows.map(r=><th key={r.yr} className="border border-gray-200 px-2 py-1.5 text-center font-semibold text-gray-600 whitespace-nowrap">{r.label}</th>)}</tr></thead>
                              <tbody>
                                {[["Revenue ($M)",r=>fmtN(r.rev),""],["Rev Growth",r=>r.yr===1?"—":`${r.revGrowth}%`,"text-gray-400"],["EBITDA ($M)",r=>fmtN(r.ebitda),"font-medium"],["EBITDA Margin",r=>`${r.ebitdaMargin}%`,""],["SBC ($M)",r=>fmtN(r.sbc),""],["SBC % Rev",r=>`${r.sbcPctRev}%`,"text-gray-400"],["EBITDA Post-SBC ($M)",r=>fmtN(r.ebitdaPostSBC),"font-medium"],["Post-SBC Margin",r=>`${r.ebitdaPostSBCMargin}%`,""],["D&A ($M)",r=>fmtN(r.da),""],["D&A % Rev",r=>`${r.daPctRev}%`,"text-gray-400"],["EBIT ($M)",r=>fmtN(r.ebit),"font-medium"],["EBIT Margin",r=>`${r.ebitMargin}%`,""],["Taxes ($M)",r=>fmtN(r.taxes),"text-gray-400"],["Tax Rate",r=>`${r.taxRate}%`,"text-gray-400"],["NOPAT ($M)",r=>fmtN(r.nopat),"font-medium"],["NOPAT Margin",r=>`${r.nopatMargin}%`,""],["(+) D&A ($M)",r=>fmtN(r.da),""],["Other Cash Flow ($M)",r=>`${r.otherCF>=0?"":"-"}${fmtN(Math.abs(r.otherCF))}`,""],["Other % Rev",r=>`${r.otherCFPctRev}%`,"text-gray-400"],["UFCF ($M)",r=>fmtN(r.ufcf),"font-medium text-blue-700"],["PV of UFCF ($M)",r=>fmtN(r.pv),"text-blue-700 font-medium"]].map(([lbl,fn,cls],i)=>(
